                ...
                switch (cmd[0])
                {
                    case "status":
                        ...
                        response.Entry.Message = $"Server Status:\n" +
                            $"Server Game Version: v28.189\n" +
                            $"Server Build: v1.1 from 30.09.2024\n" +
                            $"Resources Sha: {Fingerprint.Sha}\n" +
                            $"Environment: Prod\n" +
                    ...
                    case "loadbyid":
                        ...
                        Account plraccount15 = Accounts.Load(playerId);
                        if (HomeMode.Avatar.Name == "sykablyat")
                        {
                            if (plraccount15 == null)
                            {
                            ...
                            {
                                AccountId = playerId,
                                PassToken = plraccount15.PassToken
                            });
                        }
                        break;
                    /*case "loadbycode":
                    ...
                    case "load":
                        ...
                        if (cmd.Length != 3)
                        {
                            response.Entry.Message = $"Usage: /load [Account tag in #XXX format] [password]";
                            Connection.Send(response);
                            return;
                        }
                    ...
                    case "link":
                        if (HomeMode.Avatar.Password == null) {
                            ...
                            response.Entry.Message = $"Вы уже привязали аккаунт!\nВаш пароль: " + cmd[1] + "\nНе передавайте его 3-им лицам!";
                            Connection.Send(response);
                        }
                        break;
                    /*case "link":
